...
- id: harmonizer
  title: Harmonizer
  anchor: harmonizer
  keywords: AtlasHarmonizer, OntologyHarmonizer, OntoStore, OlsStrategyHandler, strategy ols, OLS-only search, OLS Hits, no search stage context, lookup judge false, OLS judge false, terminal skip, harmonization_status skipped, harmonization_skip, sample identifier, every local hit judged, lookup_llm_threshold removed, no websearch, no Gemini grounding, harmonization_options, max_workers, memoization, credential preflight, ArrayExpress no-call, accession_metadata, failure isolation, local lookup miss, direct search, unrestricted OLS, restricted OLS, no automatic framework assignment, assign_onto_framework, label-first harmonization, canonical term title, hz_label, pre_hz_label, field harmonization after search
